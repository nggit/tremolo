...
for i in range(len(sys.argv)):
    if sys.argv[i - 1] == '--help':
        ...
        print('  --bind                    Address to bind. Instead of using --host or --port')
        print('                            E.g. "127.0.0.1:8000"')
        print('  --reuse-port              Use SO_REUSEPORT when available')
        print('  --worker-num              Number of worker processes. Defaults to 1')
        ...
        print('  --debug                   Enable debug mode. Intended for development')
        print('  --log-level               Defaults to "DEBUG". See')
        print('                            https://docs.python.org/3/library/logging.html#levels')
        ...
        options[sys.argv[i - 1].lstrip('-').replace('-', '_')] = sys.argv[i]
                             ...
                             '--keepalive-timeout'):
        try:
            options[sys.argv[i - 1].lstrip('-').replace('-', '_')] = int(sys.argv[i])
        except ValueError:
            print('Invalid {:s} value "{:s}". It must be a number'.format(sys.argv[i - 1], sys.argv[i]))
            sys.exit(1)
